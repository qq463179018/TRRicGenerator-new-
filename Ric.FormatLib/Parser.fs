    ...
    let GetStringFromExpr value (properties: Dictionary<string, string>) =
        try
            let rec GetStringFromExprRec value (properties: Dictionary<string, string>) =
                ...
                | VarDate(date, format)     -> DateTime.ParseExact(properties.Item(GetStringFromExprRec date properties), "dd/MM/yyyy", null)
                                                .ToString(GetStringFromExprRec format properties)
