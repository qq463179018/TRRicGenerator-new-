...
type NdaFormat() =
    ...
    member private this.LoadCsv path = 
        try
            this.Template <-
                path
                |> File.ReadLines
                |> Seq.map (fun line -> NdaLine(Array.toList(line.Split ',')))
                |> Seq.toList
            ()
        with
    ...
    member public this.GenerateLines () = 
        let GetInsideList = function
            | NdaTitles(lineList)-> ()
            | NdaLine(lineList)  -> this.AddToTemplateLine lineList
    ...
    member public this.GenerateTitles () = 
        let GetInsideList = function
            | NdaTitles(lineList)-> this.AddToTemplateLine lineList
            | NdaLine(lineList)  -> ()
