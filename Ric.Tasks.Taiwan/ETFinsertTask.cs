...
namespace Ric.Tasks.Taiwan
{
    ...
    class ETFinsertTask : GeneratorBase
    {
        ...
        protected override void Start()
        {
            try
            {
                LogMessage("Grabbing data from web");
                List<ETFinsertTaskTemplate> res = GrabDataFromWeb();
                LogMessage("Store in database");
                storeInDb(res);
                LogMessage("Starting to generate files");
                GenerateFiles(res);
                GenerateFileFive();
            }
            catch (Exception ex)
        ...
        private IEnumerable<ETFinsertTaskTemplate> GrabDataFromSiteOne()
        {
            ...
            return (from tr in trs
                    select tr.SelectNodes("./td")
                        ...
                        {
                            Code = tds[0].InnerText.TrimStart().TrimEnd(),
                            ReferenceValue = tds[2].InnerText.TrimStart().TrimEnd(),
                            EstimatedValue = tds[3].InnerText.TrimStart().TrimEnd()
                        }).ToList();
        }
