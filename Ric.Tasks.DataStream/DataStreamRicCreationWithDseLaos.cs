...
namespace Ric.Tasks.DataStream
{
    #region [Config]
    [ConfigStoredInDB]
    class DataStreamRicCreationWithDseLaosConfig
    {
        [StoreInDB]
        [Category("Path")]
        ...
        public DataStreamRicCreationWithDseLaosConfig()
        {
    ...
    class DataStreamRicCreationWithDseLaos : GeneratorBase
    {
        private static DataStreamRicCreationWithDseLaosConfig configObj = null;
        ...
        {
            configObj = Config as DataStreamRicCreationWithDseLaosConfig;
        ...
        protected override void Start()
        {
            ...
            List<LaosTemplate> listLaosTemplate = FormatTemplates(listDSRicCreation);
            Logger.Log(string.Format("format records count:{0}",
                listLaosTemplate == null ? "0" : listLaosTemplate.Count.ToString()));

            LogMessage("start generate output file ...");
            GenetateFile(listLaosTemplate);

            LogMessage("task finished!");
        }

        #region [generate bulk file]
        private void GenetateFile(List<LaosTemplate> listLaosTemplate)
        {
            if (listLaosTemplate == null || listLaosTemplate.Count == 0)
            {
                Logger.Log("listLaosTemplate.count==0, no data need to ouput. ", Logger.LogType.Warning);
                return;
            }
            ...
            try
            {
                ...
                string filePath = Path.Combine(fileFolder, string.Format("Laos_{0}.MAC", configObj.Date));

                foreach (var item in listLaosTemplate)
                {
                ...
                string templateString = File.ReadAllText(@"Config\DataStream\Laos.MAC", Encoding.ASCII);
        ...
        private List<LaosTemplate> FormatTemplates(List<DataStreamRicCreationEntity> listDSRicCreation)
        {
            List<LaosTemplate> list = new List<LaosTemplate>();
            ...
            try
            {
                ...
                {
                    LaosTemplate template = new LaosTemplate(item);
                    //Sedol 
        ...
        private void FormatCName(LaosTemplate bulkTemp, string names)
        {
        ...
        private bool IsValidLaosTemplate(DataStreamRicCreationEntity item)
        {
            bool result = true;
            try
            {
