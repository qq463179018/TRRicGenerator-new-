...
namespace Ric.Tasks.DataStream
{
    #region [Config]
    [ConfigStoredInDB]
    class DataStreamRicCreationWithDseNewZealandConfig
    {
        [StoreInDB]
        [Category("Path")]
        ...
        public DataStreamRicCreationWithDseNewZealandConfig()
        {
    ...
    class DataStreamRicCreationWithDseNewZealand : GeneratorBase
    {
        private static DataStreamRicCreationWithDseNewZealandConfig configObj = null;
        ...
        {
            configObj = Config as DataStreamRicCreationWithDseNewZealandConfig;
        ...
        protected override void Start()
        {
            ...
            List<NewZealandTemplate> listNewZealandTemplate = FormatTemplates(listDSRicCreation);
            Logger.Log(string.Format("format records count:{0}",
                listNewZealandTemplate == null ? "0" : listNewZealandTemplate.Count.ToString()));

            GenetateFile(listNewZealandTemplate);
        }

        #region [generate bulk file]
        private void GenetateFile(List<NewZealandTemplate> listNewZealandTemplate)
        {
            if (listNewZealandTemplate == null || listNewZealandTemplate.Count == 0)
            {
                Logger.Log("listNewZealandTemplate.count==0, no data need to ouput. ", Logger.LogType.Warning);
                return;
            }
            ...
            try
            {
                ...
                string filePath = Path.Combine(fileFolder, string.Format("NewZealand_{0}.MAC", configObj.Date));

                foreach (var item in listNewZealandTemplate)
                {
                ...
                string templateString = File.ReadAllText(@"Config\DataStream\NewZealand.MAC", Encoding.ASCII);
        ...
        private List<DataStreamRicCreationEntity> FormatRecords(List<string> listExtractedRecords)
        {
            ...
            try
            {
                ...
                foreach (var record in listExtractedRecords)
                {
                    ...
                    //dsInfo.fileType = string.Empty;
                    //dsInfo.roundLotSize = string.Empty;
                    //dsInfo.marketSegmentName = string.Empty;
                    //dsInfo.originalRecord = string.Empty;
            ...
            try
            {
                ...
                using (StreamReader sr = new StreamReader(item))
                {
                    ...
                    while ((line = sr.ReadLine().ToUpper()) != null)
                    {
                        ...
                        //{
                        //    exchangeCode = line.Substring(98, 3);

                        //    if (!exchangeCode.Equals("NZC"))
                        //        continue;
                        //} 
    ...
    public class NewZealandTemplate
    {
