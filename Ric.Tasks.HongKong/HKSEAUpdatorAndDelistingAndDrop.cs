    ...
    public class HKSEAUpdatorAndDelistingAndDrop : GeneratorBase
    {
        ...
        private void UpdateFiles()
        {
            ...
            using (ExcelApp app = new ExcelApp(false, false))
            {
                ...
                if (worksheet5 == null)
                {
                    LogMessage(string.Format("Cannot get worksheet5 from workbook {0}", workbook.FullName));
                }
                ...
                {
                    LogMessage(string.Format("Cannot get worksheet ADD DROP from workbook {0}", workbook.FullName));
                }
        ...
        private void GetRevisedRecord(string revisedXlsFilePath, out List<ExtraChangingInfo> revisedRecordList)
        {
            ...
            if (File.Exists(revisedXlsFilePath))
            {
                ...
                using (ExcelApp app = new ExcelApp(false, false))
                {
                    ...
                    if (worksheet == null)
                    {
                        LogMessage(string.Format("Cannot get worksheet {0} from workbook {1}", configObj.RevsiedSourceFileWorksheetName, configObj.RevisedSourceFilePath));
                    }
        ...
        private void GetCancelledRecord(string cancelledXlsFilePath, out List<ExtraChangingInfo> cancelledRecordList)
        {
            ...
            if (File.Exists(cancelledXlsFilePath))
            {
                ...
                using (ExcelApp app = new ExcelApp(false, false))
                {
                    ...
                    if (worksheet == null)
                    {
                        LogMessage(string.Format("Cannot get worksheet {0} from workbook {1}", configObj.CancelledSourceFileWorksheetName, configObj.CancelledSourceFilePath));
                    }
                    using (ExcelLineWriter reader = new ExcelLineWriter(worksheet, 16, 3, ExcelLineWriter.Direction.Right))
                    {
                        int endRowPos = worksheet.UsedRange.Row + worksheet.UsedRange.Rows.Count - 1;
            ...
            out List<SpecialTradingInfo> bigRicParallelTradingListTwo, out List<SpecialTradingInfo> firstTypeSingleTradingList)
        {
            ...
            using (ExcelApp app = new ExcelApp(false, false))
            {
                ...
                if (worksheet == null)
                {
                    LogMessage(string.Format("Cannot get worksheet {0} from workbook {1}", xlsFilePath, configObj.FormalSourceFilePath));
                }
        ...
        public bool IsIPos(string ricStr)
        {
            ...
            if (!result)
            {
                LogMessage(string.Format("Can't parse {0}", ricStr));
            }
            else
            {
        ...
        public Range GetAKRange(Workbook workbookSource, out int lastUsedRowNum, out List<BordLotSizeChangeInfo> bordLotSizeChangeList)
        {
            ...
            {
                LogMessage(string.Format("Cannot get worksheet {0} from workbook {1}", configObj.FormalSourceFileWorksheetName, workbookSource.Name));
            }
        ...
        public void UpdateTemplateV2File(Range A2Range, int lastRowContainInfo)
        {
            ...
            using (ExcelApp appTemplateV2 = new ExcelApp(false, false))
            {
                ...
                {
                    LogMessage(string.Format("Cannot get worksheet {0} from workbook {1}", configObj.TemplateV2WorksheetName, workbookTemplateV2.Name));
                }
        ...
        private void StartDelistingAndNameChangeJob()
        {
            ...
            if (File.Exists(formalFileBackupPath))
            {
                ...
                using (ExcelApp app = new ExcelApp(false, false))
                {
                    ...
                    if (worksheet == null)
                    {
                        LogMessage(string.Format("Cannot get worksheet {0} from workbook {1}", configObj.FormalSourceFileWorksheetName, formalFileBackupPath));
                    }
            ...
            {
                LogMessage(string.Format(" Can't find today's delisting file {0}!", configObj.FormalSourceFilePath));
            }
        }
        ...
        private void GetInfoFromInternet(string code, string condition)
        {
            ...
            LogMessage("Get HTML raw data error!");
        }
