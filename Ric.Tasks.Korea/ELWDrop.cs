    ...
    public class ELWDrop : GeneratorBase
    {
        ...
        private void GetISINFromWebpage()
        {
            try
            {
                ...
                //------WebKitFormBoundaryrPcf1lplEzyEtUru
                //Content-Disposition: form-data; name="com_nm11"

                //ëŒ€ì‹ 
                //------WebKitFormBoundaryrPcf1lplEzyEtUru
                //Content-Disposition: form-data; name="isur_nm11"

                //ì œ4043í˜¸
                //------WebKitFormBoundaryrPcf1lplEzyEtUru
                //Content-Disposition: form-data; name="isu_start_dd11"
                ...
                foreach (var item in dropList)
                {
                    ...
                    postData = File.ReadAllText(@"Config\Korea\ELWDropPostData.txt", Encoding.Unicode);
                    postData = string.Format(postData, item.Issuername, item.Num, item.Issuername, item.Num);
                    //AdvancedWebClient wc = new AdvancedWebClient();
                    //string pageSource = WebClientUtil.GetPageSource(wc, uri, 300000, postData);
                    //HtmlAgilityPack.HtmlDocument doc = new HtmlAgilityPack.HtmlDocument();
                    HtmlDocument doc = GetDocFromELW(uri, postData);
                    //if (!string.IsNullOrEmpty(doc))
                    //    doc.LoadHtml(pageSource);
                    if (doc != null)
                    {
                        //HtmlNode node = doc.DocumentNode.SelectNodes("//table/tr/td/table/tr")[1].SelectNodes("td")[1];
                        var nodeList = doc.DocumentNode.SelectNodes(".//table")[1].SelectNodes(".//tr");

                        string isin = string.Empty;
                        if (nodeList != null && nodeList.Count >= 1)
                            isin = nodeList[1].SelectNodes(".//td")[1].InnerText.Replace("&nbsp;", "").Replace("\n", "").Replace("\r", "").Replace("\t", "").Trim();
                        if (!string.IsNullOrEmpty(isin))
                            item.ISIN = isin;
                    }
                }
        ...
        private HtmlDocument GetDocFromELW(string url, string post)
        {
            ...
            request.ContentType = "multipart/form-data; boundary=----WebKitFormBoundaryad0mB4VHXSa6m8hz";
        ...
        private void GenerateDropGeda()
        {
            ...
            AddResult(Path.GetFileNameWithoutExtension(filePath),filePath,"DROP GEDA File");
        }
        #region Drop Nda
        ...
        private void GenerateIA()
        {
            ...
            FileUtil.WriteOutputFile(filePath, res, title, WriteMode.Append);
            AddResult(Path.GetFileNameWithoutExtension(filePath),filePath,"DROP NDA IA File");
        ...
        private void GenerateQA()
        {
            ...
            if (!updatedTagPilc)
            {
                System.Windows.Forms.MessageBox.Show("Please update the TAG and PILC file and run this task again.");
            }

            FileUtil.WriteOutputFile(filePath, res, title, WriteMode.Append);
            AddResult(Path.GetFileNameWithoutExtension(filePath),filePath,"DROP NDA QA File");
        }
        #endregion
