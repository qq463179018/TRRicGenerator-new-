...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDsePeru : GeneratorBase
    {
        #region Fields
        ...
        private void DownloadFtpFiles()
        {
            ...
            //特定的文件头
            string[] fileStartArr = new string[] { "0393", "EM03", "EM13" };
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                if (item.fileType.StartsWith("EM"))
                {
                    ...
                    if (exchangeCode.CompareTo("LMA") != 0)
                    {
                        continue;
                    }
                }
                string scheme = item.ThomsonReutersClassificationScheme;
                if (scheme.CompareTo("RTS") != 0 && scheme.CompareTo("ETF") != 0 && scheme.CompareTo("CEF") != 0)
                {
        ...
        private string GetMnem(string ric,string assetCategory)
        {
            string mnem = string.Empty;
            if (assetCategory.CompareTo("PRF") == 0 || assetCategory.CompareTo("CPR") == 0)
            {
                mnem = ric.Substring(0, 1);
                int a = ric.LastIndexOf('.');
                if (a > 0)
                {
                    mnem += ric.Substring(a - 1, 1);
                }
                else
                {
                    mnem += ' ';
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                bulkTemp.Mnem = "PE:" + GetMnem(item.RIC,item.AssetCategory);
        ...
        private void GenerateFile(string lineToFile)
        {
            try
            {
                ...
                string fileName = string.Format("Peru_{0}.MAC", ConfigObj.Date);
