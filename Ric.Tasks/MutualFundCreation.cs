...
namespace Ric.Generator.Lib.Validation
{
    #region Config
    ...
    public class MutualFundCreation : GeneratorBase
    {
        ...
        protected override void Initialize()
        {
            ...
            try
            {
                service = EWSUtility.CreateService(new NetworkCredential(configObj.MailUsername, configObj.MailPassword, configObj.MailDomain), new Uri(@"https://apac.mail.erf.thomson.com/EWS/Exchange.asmx"));
                app = new Util.ExcelApp(false, false);
        ...
        protected override void Start()
        {
            ...
            try
            {
                ...
                ReadExcel();
                LoginToWebsite();
                foreach (MutualFund fund in funds.Where(fund => fund.Loc.Length == 9))
                {
                    Search(fund.Loc);
                    GetSummary();
        ...
        private MutualFund GetMutualFundFromLine(Range range)
        {
            ...
            {
                BDate = DateTime.FromOADate(Convert.ToDouble(range.Range["C1"].Value2.ToString())),
                FundFamily = range.Range["Q1"].Value2.ToString(),
                Loc = range.Range["M1"].Value2.ToString(),
                Mnem = range.Range["K1"].Value2.ToString(),
                Name = range.Range["O1"].Value2.ToString()
            };
        ...
        private string Abbreviate(string name)
        {
            ...
            name = keysFound.Aggregate(name, (current, key) => current.Replace(key, abbr[key]));
