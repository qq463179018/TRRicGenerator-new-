    ...
    public class ELWFMFirstPart : GeneratorBase
    {
        ...
        private void GrabDataFromWebpage()
        {
            try
            {
                ...
                pageCount = GetTotalPageCount(uri, startDate, endDate);
                    ...
                    if (htc != null)
                    {
                        ...
                        {                           
                            for (int x = 1; x < nodeList.Count - 1; x++)
                            {
                                ...
                                HtmlNode tdNode = nodeList[x].SelectSingleNode(".//td/a") as HtmlNode;
                                string isin = tdNode.InnerText.Trim().ToString();
                                string attribute = tdNode.Attributes["href"].Value.ToString().Trim();
                                ...
                                string url = string.Format("http://isin.krx.co.kr/jsp/BA_VW021.jsp?isu_cd={0}&modi=t&req_no={1}", param2, param1);
                                HtmlDocument doc = WebClientUtil.GetHtmlDocument(url, 300000);
                                if (doc != null)
                                {
                                    HtmlNode table = doc.DocumentNode.SelectNodes("//table")[2];
                                    string tr5_td2 = table.SelectSingleNode(".//tr[5]/td[2]").InnerText.Trim().ToString();
                                    if (tr5_td2.Equals("사모"))
                                    {
                                        string msg = string.Format("ISIN:{0} equals 사모. Ignore it.", isin);
                                        Logger.Log(msg);
                                        continue; 
                                    }
                                    warrantTemp.ISIN = isin;
                                    GrabDataAndWarrantTemplateFormat(warrantTemp, table);
        ...
        private int GetTotalPageCount(string uri, string startDate, string endDate)
        {
            int pageCount = 0;
            try
            {
                string postData = string.Format("kind=&ef_std_cd_grnt_dt_from={0}&ef_std_cd_grnt_dt_to={1}&secuGubun=07&lst_yn_all=on&lst_yn1=Y&lst_yn2=N&lst_yn3=D&els_dls_all=on&els_dls1=els&els_dls2=dls&so_gb_all=on&so_gb1=s&so_gb2=o&jp_gb_all=on&jp_gb1=c&jp_gb2=t&jp_gb3=r&jp_gb4=i&hg_gb_all=on&hg_gb1=h&hg_gb2=g&tg_gb_all=on&tg_gb1=x&tg_gb2=z&df_gb_all=on&df_gb1=df1&df_gb2=df2&df_gb3=df3&df_gb4=df4&df_gb5=df5&df_gb6=df6&df_gb7=df7&cb_search_column=co_nm&ef_key_word=&ef_iss_inst_cd=&ef_isu_nm=&ef_iss_dt_from=&ef_iss_dt_to=&ef_lst_dt_from=&ef_lst_dt_to=", startDate, endDate);
                string pageSource = string.Empty;
                HtmlDocument htc = new HtmlDocument();
                AdvancedWebClient wc = new AdvancedWebClient();
                pageSource = WebClientUtil.GetPageSource(wc, uri, 300000, postData);
        ...
        private void GrabDataAndWarrantTemplateFormat(WarrantTemplate warrantTemp, HtmlNode table)
        {
            try
            {
                ...
                string tr2_td4 = table.SelectSingleNode(".//tr[2]/td[4]").InnerText.Trim().ToString();
                string tr3_td4 = table.SelectSingleNode(".//tr[3]/td[4]").InnerText.Trim().ToString();
                string tr4_td2 = table.SelectSingleNode(".//tr[4]/td[2]").InnerText.Trim().ToString();
                string tr4_td4 = table.SelectSingleNode(".//tr[4]/td[4]").InnerText.Trim().ToString();
                string tr6_td2 = table.SelectSingleNode(".//tr[6]/td[2]").InnerText.Trim().ToString();
                string tr6_td4 = table.SelectSingleNode(".//tr[6]/td[4]").InnerText.Trim().ToString();
                string tr7_td2 = table.SelectSingleNode(".//tr[7]/td[2]").InnerText.Trim().ToString();
                string tr7_td4 = table.SelectSingleNode(".//tr[7]/td[4]").InnerText.Trim().ToString();
                string tr8_td2 = table.SelectSingleNode(".//tr[8]/td[2]").InnerText.Trim().ToString();
                string tr11_td4 = table.SelectSingleNode(".//tr[11]/td[4]").InnerText.Trim().ToString();
                string tr13_td2 = table.SelectSingleNode(".//tr[13]/td[2]").InnerText.Trim().ToString();
                ...
                warrantTemp.StrikePrice = tr20_td4.Contains(",") ? tr20_td4.Replace(",", "") : tr20_td4;
        ...
        private void CopyELWFM1FileToEmaFile()
        {
            ...
            try
            {
                ...
                excelApp.ExcelAppInstance.AlertBeforeOverwriting = false;
                wBook.Save();
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(emaPath), "ADD FM File(EMA)", emaPath, FileProcessType.Other));
            }
            catch (Exception ex)
            {
                string msg = "Error found in GenerateELWFM1File_xls()    : " + ex.StackTrace + " \r\n" + ex.ToString();
        ...
        private void GenerateNDAIAFileXls(string ipath)
        {
            ...
            try
            {
                ...
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(ipath), "ADD NDA IA File", ipath, FileProcessType.NDA));
            }
            catch (Exception ex)
            {
        ...
        private void GenerateNDAQAFileXls(string ipath)
        {
            ...
            try
            {
                ...
                excelApp.ExcelAppInstance.AlertBeforeOverwriting = false;
                wBook.SaveAs(wBook.FullName, XlFileFormat.xlCSV, System.Type.Missing, System.Type.Missing, System.Type.Missing, System.Type.Missing, XlSaveAsAccessMode.xlExclusive, System.Type.Missing, System.Type.Missing, System.Type.Missing, System.Type.Missing, System.Type.Missing);
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(ipath), "ADD NDA QA File", ipath, FileProcessType.NDA));
            }
            catch (Exception ex)
            {
        ...
        //        excelApp.ExcelAppInstance.AlertBeforeOverwriting = false;
        //        wBook.SaveAs(wBook.FullName, XlFileFormat.xlCSV, System.Type.Missing, System.Type.Missing, System.Type.Missing, System.Type.Missing, XlSaveAsAccessMode.xlExclusive, System.Type.Missing, System.Type.Missing, System.Type.Missing, System.Type.Missing, System.Type.Missing);
        //        TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(ipath), "", ipath, FileProcessType.NDA));
        //    }
        //    catch (Exception ex)
        //    {
        //        string msg = "Error found in NDA QA file :" + ex.ToString();
        ...
        private void GenerateNDATCFile_xls(string ipath)
        {
            ...
            try
            {
                ...
                excelApp.ExcelAppInstance.AlertBeforeOverwriting = false;
                wBook.SaveAs(wBook.FullName, XlFileFormat.xlCSV, System.Type.Missing, System.Type.Missing, System.Type.Missing, System.Type.Missing, XlSaveAsAccessMode.xlExclusive, System.Type.Missing, System.Type.Missing, System.Type.Missing, System.Type.Missing, System.Type.Missing);
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(ipath), "ADD NDA TC File(EMA)", ipath, FileProcessType.Other));
            }
            catch (Exception ex)
            {
        ...
        private void GenerateKSEELWFileTxt(string ipath)
        {
            try
            {
                ...
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(ipath), "KSE ELW File", ipath, FileProcessType.GEDA_BULK_RIC_CREATION));
            }
            catch (Exception ex)
            {
        ...
        private void GenerateNewUnderlyingFiles()
        {
            ...
            //From Config
            TaskResultList.Add(new TaskResultEntry("New Underlying Folder", "New Underlying Folder", configObj.GEDA_NewUnderlying));
            ...
            TaskResultList.Add(new TaskResultEntry(uploadFileName, @"GEDA File: UNDERLYING_CHAIN_UPLOAD", Path.Combine(filePath, uploadFileName), FileProcessType.Other));              
            TaskResultList.Add(new TaskResultEntry(chainAddFileName, @"GEDA File: CHAIN_CONST_ADD", Path.Combine(filePath, chainAddFileName), FileProcessType.Other));
            if (superKQ)
            {
                TaskResultList.Add(new TaskResultEntry(superKQFileName, @"GEDA File: SUPERCHAIN_CONST_ADD", Path.Combine(filePath, superKQFileName), FileProcessType.Other));
            }
            if (superKS)
            {
                TaskResultList.Add(new TaskResultEntry(superKSFileName, @"GEDA File: SUPERCHAIN_CONST_ADD", Path.Combine(filePath, superKSFileName), FileProcessType.Other));
            }
        ...
        private void CreatMailAndSave(string mailSubject, string mailBody, string filePath)
        {
            ...
            TaskResultList.Add(new TaskResultEntry(mail.MailSubject + ".msg", mail.MailSubject, filePath, mail));
        }
