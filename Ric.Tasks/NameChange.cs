...
namespace Ric.Tasks
{
    public class NameChange : GeneratorBase
    {
        ...
        private KOREA_NameChangeGeneratorConfig configObj;
        ...
        private void GrabDataFromWebpage()
        {
            ...
            string dataStartDate = startDay.CompareTo(twoMonthEarly) < 0 ? startDate : twoMonthEarly.ToString("yyyy-MM-dd");
            ...
            if (htc != null)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    try
                    {
                        ...
                        if (node != null)
                            title = node.SelectSingleNode(".//span/a").InnerText.Trim();
        ...
        private KoreaEquityInfo GrabIsinInfo(HtmlNode judgeNode)
        {
            try
            {
                string judgeText = judgeNode.Attributes["onclick"].Value.Trim();
        ...
        private string GetTickerByAttribute(HtmlNode node)
        {
            ...
            try
            {
                string attribute = node.Attributes["onclick"].Value.Trim();
        ...
        private void GrabCommonKQDataChangeAction(KoreaEquityInfo item, HtmlDocument sourceNode)
        {
            try
            {
                ...
                foreach (var node in tables)
                {
                    ...
                    if (knameNode != null)
                        if (knameNode.InnerText.Trim().Equals("한글"))
                            table = node;
                }
        ...
        private void GrabEnglishCompanyChangeData(KoreaEquityInfo item, HtmlDocument sourceNode)
        {
            try
            {
                string strPre = sourceNode.DocumentNode.SelectSingleNode(".//pre").InnerText.Trim();
        ...
        private void GrabCommonKSData(HtmlDocument sourceNode, KoreaEquityInfo equity)
        {
            try
            {
                ...
                foreach (MatchCollection match in strIsinArr.Select(str => str.Trim())
                                                            .Where(temp => temp.Contains("표준코드") 
                         && temp.Contains("단축코드")).Select(temp => temp.Substring(temp.IndexOf("표준코드")))
                                                      .Select(temp => regex.Matches(temp))
                                                      .Where(match => match.Count == 2))
                {
        ...
        private void GrabLegalNameDataChangeAction(KoreaEquityInfo item, HtmlDocument sourceNode)
        {
            ...
            {
                HtmlNodeCollection tables = sourceNode.DocumentNode.SelectNodes(".//table");
                HtmlNode table = (from node in tables 
                                  let knameNode = node.SelectSingleNode(".//tr[2]/td[1]") 
                                  where knameNode != null 
                                  where knameNode.InnerText.Trim().Contains("변경내역") 
        ...
        private void FormatNameChangeTemplate(IEnumerable<KoreaEquityInfo> list)
        {
        ...
        private void UpdateChanges(KoreaEquityInfo item)
        {
            ...
            try
            {
                ...
                if (row > 0)
                {
                    Logger.Log(string.Format("Update {0} record in Korea Equity Table. RIC:{1}", row, item.RIC));
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                foreach (var item in list)
                {
                    ...
                    if (item.ISIN.Length != 12)
                        ((Range)wSheet.Cells[row, 6]).Font.Color = ColorTranslator.ToOle(Color.Red);
        ...
        private void GenerateExcelFileTitle(Worksheet wSheet)
        {
            if (wSheet.get_Range("C1", Type.Missing).Value2 == null)
            {
                ...
                ((Range)wSheet.Columns["C", Type.Missing]).ColumnWidth = 15;
                ((Range)wSheet.Columns["D", Type.Missing]).ColumnWidth = 15;
                ((Range)wSheet.Columns["E", Type.Missing]).ColumnWidth = 18;
                ((Range)wSheet.Columns["F", Type.Missing]).ColumnWidth = 18;
                ((Range)wSheet.Columns["G", Type.Missing]).ColumnWidth = 15;
                ((Range)wSheet.Columns["H", Type.Missing]).ColumnWidth = 15;
                ((Range)wSheet.Columns["I", Type.Missing]).ColumnWidth = 30;
                ((Range)wSheet.Columns["J", Type.Missing]).ColumnWidth = 30;
                ((Range)wSheet.Columns["K", Type.Missing]).ColumnWidth = 20;
                ((Range)wSheet.Columns["L", Type.Missing]).ColumnWidth = 25;
                ((Range)wSheet.Columns["M", Type.Missing]).ColumnWidth = 20;
                ((Range)wSheet.Columns["A:M", Type.Missing]).Font.Name = "Arial";

                //fill the cells with color 
                ((Range)wSheet.Cells[1, 1]).Interior.Color = ColorTranslator.ToOle(Color.Yellow);
                ((Range)wSheet.Cells[1, 2]).Interior.Color = ColorTranslator.ToOle(Color.Pink);
                ((Range)wSheet.Cells[1, 3]).Interior.Color = ColorTranslator.ToOle(Color.Yellow);
                ((Range)wSheet.Cells[1, 4]).Interior.Color = ColorTranslator.ToOle(Color.Pink);
                ((Range)wSheet.Cells[1, 5]).Interior.Color = ColorTranslator.ToOle(Color.Yellow);
                ((Range)wSheet.Cells[1, 6]).Interior.Color = ColorTranslator.ToOle(Color.Pink);
                ((Range)wSheet.Cells[1, 7]).Interior.Color = ColorTranslator.ToOle(Color.Yellow);
                ((Range)wSheet.Cells[1, 8]).Interior.Color = ColorTranslator.ToOle(Color.Pink);
                ((Range)wSheet.Cells[1, 9]).Interior.Color = ColorTranslator.ToOle(Color.Yellow);
                ((Range)wSheet.Cells[1, 10]).Interior.Color = ColorTranslator.ToOle(Color.Pink);
                ((Range)wSheet.Cells[1, 11]).Interior.Color = ColorTranslator.ToOle(Color.Yellow);
                ((Range)wSheet.Cells[1, 12]).Interior.Color = ColorTranslator.ToOle(Color.Pink);
                ((Range)wSheet.Cells[1, 13]).Interior.Color = ColorTranslator.ToOle(Color.Yellow);
        ...
        private void GenerateGEDANDAFiles()
        {
            ...
            foreach (KoreaEquityInfo info in nameChanges)
            {
                CreateGEDAFile(info, configObj.GEDA);
                CreateNDAIAFile(info, configObj.NDA);
                CreateNDAQAFile(info, configObj.NDA);
            }
        }
        ...
        private void CreateTaskResult()
        {
            ...
            foreach (string fileName in resultOrder)
            {
                try
                {
                    var newResult = filesResultList[fileName] as TaskResultEntry;
                    AddResult(newResult.Description, newResult.Path, "file");
