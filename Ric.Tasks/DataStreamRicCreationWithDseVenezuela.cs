...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDseVenezuela : GeneratorBase
    {
        #region Fields
        ...
        private void DownloadFtpFiles()
        {
            ...
            //特定的文件头
            string[] fileStartArr = new string[] { "0199", "EM03", "EM13" };
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                if (item.AssetCategory.CompareTo("OPF") == 0)
                {
                    continue;
                }
                ...
                if (item.fileType.StartsWith("EM"))
                {
                    ...
                    if (exchangeCode.CompareTo("CCS") != 0)
                    {
                        continue;
                    }
                }
                if (item.ThomsonReutersClassificationScheme.CompareTo("RTS") != 0 && item.ThomsonReutersClassificationScheme.CompareTo("ETF") != 0)
                {
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                bulkTemp.Mnem = "V:" + GetMnem(item.Ticker).PadRight(4,' ');
                ...
                bulkTemp.Nosh = "1";
                bulkTemp.Mnem2 = "VE" + item.Ticker.PadRight(10, ' ');
        ...
        private void GenerateFile(string lineToFile)
        {
            try
            {
                ...
                string fileName = string.Format("Venezuela_{0}.MAC", ConfigObj.Date);
