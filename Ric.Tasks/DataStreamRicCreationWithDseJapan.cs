...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDseJapan : GeneratorBase
    {
        #region Fields
        ...
        private void DownloadFtpFiles()
        {
            ...
            //特定的文件头
            string[] fileStartArr = new string[] { "0002","0004","0006","0007", "EM01", "EM11" };
        ...
        private Dictionary<string, DseFieldPosition> InitializeDseFields()
        {
            ...
            dseFields.Add("Malaysia code", new DseFieldPosition(301, 310));
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                item.malaysiaCode = FormatField(record, "Malaysia code", dseFields);
                item.fileType = record.DseFileType;

                if (item.fileType.StartsWith("EM"))
                {
                    string exchangeCode = item.ExchangeCode;
                    if (exchangeCode.CompareTo("TYO") != 0 && exchangeCode.CompareTo("NGO") != 0 && exchangeCode.CompareTo("FKA") !=0 && exchangeCode.CompareTo("SAP") != 0)
                    {
                        continue;
                    }
                }
                if (item.ThomsonReutersClassificationScheme.CompareTo("RTS") != 0)
                {
        ...
        private string GetMnem(string companyName)
        {
            ...
            else if (tmp.Length == 3)
            {
                ...
                mnem += tmp[2].Substring(tmp[0].Length - 1, 1);
            }
            else if (tmp.Length == 2)
            {
                mnem += tmp[0].Substring(0, 1);
                mnem += tmp[0].Substring(tmp[0].Length - 1, 1);
                mnem += tmp[1].Substring(0, 1);
                mnem += tmp[1].Substring(tmp[1].Length - 1, 1);
            }
            else if (tmp.Length == 1)
            {
                if (tmp[0].Length >= 4)
                {
                    mnem += tmp[0].Substring(0, 3);
                    mnem += tmp[0].Substring(tmp[0].Length - 1, 1);
                }
                else
                {
                    mnem += tmp[0].PadRight(4,' ');
                }
            }
            return mnem;
        }
        ...
        {
            string[] endings = new string[] { "JSC", "JOINT STOCK COMPANY", "JOINT STOCK CO", "JOINT STOCK CORPORATION", "JOINT-STOCK CORPORATION", "PUBLIC LIMITED COMPANY", "INCORPORATED", "COMPANY", "LIMITED", "LTD", "CORPORATION", "CO.", "CORP", "INC", "LLC", "TBK", "TERBUKA", "PLC.", "PLC", "Public Limited Company", "Public Limited Co", "Public Limited Co.", "BHD", "BERHAD" };
            foreach (string ending in endings)
            {
                if ((nameLeft.Length > ending.Length) && ((nameLeft.Substring(nameLeft.Length - ending.Length - 1).Equals(" " + ending)) || (nameLeft.Substring(nameLeft.Length - ending.Length - 1).Equals("." + ending))))
                {
                    ...
                    else if (ending.Equals("BHD") || ending.Equals("BERHAD"))
                    {
                        nameSuffix = "BHD.";
                    }
                }
            ...
            if (nameLeft.Contains(" BERHAD "))
            {
                nameLeft = nameLeft.Replace("BERHAD ", "BHD.");
            }
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                bulkTemp.Mnem = "J:" + GetMnem(item.CompanyName);
                ...
                if (item.ExchangeCode.CompareTo("TYO") == 0)
                {
                    ...
                    else if (item.SecurityLongDescription.Contains("REIT"))
                    {
                        bulkTemp.Remk = "REIT";
                        bulkTemp.DefMki = "TY";
                    }
                    else if (item.marketSegmentName.CompareTo("Tokyo SE 1st Section") == 0)
                    {
                        bulkTemp.Remk = "1ST ";
                        bulkTemp.DefMki = "TY";
                    }
                    else if (item.marketSegmentName.CompareTo("Tokyo SE 2nd Section") == 0)
                    {
                        bulkTemp.Remk = "2ND ";
                        bulkTemp.DefMki = "TY";
                    }
                    else if (item.marketSegmentName.CompareTo("Tokyo SE Mothers") == 0)
                    {
                        bulkTemp.Remk = "MOTH";
                        bulkTemp.DefMki = "TY";
                    }
                    else if (item.marketSegmentName.CompareTo("TSE JASDAQ Standard") == 0)
                    {
                        bulkTemp.Remk = "OTC ";
                        bulkTemp.DefMki = "JO";
                    }
                ...
                else if (item.ExchangeCode.CompareTo("NGO") == 0)
                {
                    if (item.marketSegmentName.CompareTo("Nagoya 1st Section") == 0)
                    {
                        bulkTemp.Remk = "1STN";
                        bulkTemp.DefMki = "NG";
                    }
                    else if (item.marketSegmentName.CompareTo("Nagoya 2nd Section") == 0)
                    {
                        bulkTemp.Remk = "2NDN";
                        bulkTemp.DefMki = "NG";
                    }
                    else if (item.marketSegmentName.CompareTo("Nagoya SE Centrex") == 0)
                    {
                        bulkTemp.Remk = "CNTX";
                        bulkTemp.DefMki = "NG";
                    }
                }
                else if (item.ExchangeCode.CompareTo("FKA") == 0)
                {
                    bulkTemp.Remk = "FKKA";
                    bulkTemp.DefMki = "FU";
                }
                else if (item.ExchangeCode.CompareTo("SAP") == 0)
                {
                    bulkTemp.Remk = "SAPA";
                    bulkTemp.DefMki = "SO";
                }
                ...
                bulkTemp.Nosh = "1";
                bulkTemp.Mnem2 = "J" + item.malaysiaCode.PadRight(11, ' ');
        ...
        private void GenerateFile(string lineToFile)
        {
            try
            {
                ...
                string fileName = string.Format("Japan_{0}.MAC", ConfigObj.Date);
        ...
        private string InitializeMacTemplate()
        {
            ...
            string templateFilePath = Directory.GetCurrentDirectory() + @"\\DataStreamTemplate\\01.MAC";
