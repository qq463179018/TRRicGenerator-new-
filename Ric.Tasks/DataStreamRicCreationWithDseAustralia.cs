...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDseAustralia:GeneratorBase
    {
        #region Fields
        ...
        private void DownloadFtpFiles()
        {
            ...
            string[] fileStartArr = new string[] { "0066", "0545","0527","0509","1402","EM01","EM11" };
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                if (item.fileType.StartsWith("EM"))
                {
                    ...
                    if (exchangeCode.CompareTo("ASX") != 0 && exchangeCode.CompareTo("CHA") != 0 && exchangeCode.CompareTo("AUX") != 0 && exchangeCode.CompareTo("AXP") != 0)
                    {
                        continue;
                    }
                }

                if ((item.ThomsonReutersClassificationScheme.CompareTo("RTS") != 0) && (item.ThomsonReutersClassificationScheme.CompareTo("STAPLED") !=0))
                {
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                bulkTemp.Mnem = GetMnem(item.Ticker,item.ThomsonReutersClassificationScheme);
                ...
                if (item.ThomsonReutersClassificationScheme.CompareTo("PRF") == 0 || item.ThomsonReutersClassificationScheme.CompareTo("CPR") == 0 || item.ThomsonReutersClassificationScheme.CompareTo("PREFERRED") == 0)
                {
                ...
                else if (item.ThomsonReutersClassificationScheme.CompareTo("DEFER") == 0)
                {
                    FormatBulkNames(item.SecurityLongDescription, 24, 20, nameInputType.Name, ref name1, ref name2, ref suffix);
                    bulkTemp.NameSuffix = suffix;
                    bulkTemp.Name1 = name1;
                    bulkTemp.Name2 = name2;
                    if (name1.Trim().Length <= 20)
                    {
                        string tmp = name1.Trim() + "DFD.";
                        bulkTemp.Name1 = tmp.PadRight(24, ' '); 
                    }
                    else
                    {
                        string tmp = name2.Trim() + "DFD.";
                        bulkTemp.Name2 = tmp.PadRight(24, ' ');
                    }
                    FormatBulkNames(item.SecurityLongDescription, 19, 20, nameInputType.Fname, ref name1, ref name2, ref suffix);
                    bulkTemp.Fname1 = name1;
                    bulkTemp.Fname2 = name2;
                    if (name1.Trim().Length <= 15)
                    {
                        string tmp = name1.Trim() + "DFD.";
                        bulkTemp.Fname1 = tmp.PadRight(19,' ');
                    }
                    else
                    {
                        string tmp = name2.Trim() + "DFD.";
                        bulkTemp.Fname2 = tmp.PadRight(24, ' ');
                    }
                }
                else
                {
                ...
                else
                {
                 ...
                 bulkTemp.Bdate = string.Empty.PadRight(8, ' ');
                ...
                bulkTemp.Nosh = "1";
                bulkTemp.Mnem2 = "A:" + item.Ticker.PadRight(10,' ');
        ...
        private string GetMnem(string ticker,string scheme)
        {
            string mnem = string.Empty;
            if (scheme.ToUpper().CompareTo("DEFER") == 0 )
            {
                mnem = "A:" + ticker.Substring(0, 3) + "Y";
            }
            else if ((scheme.ToUpper().CompareTo("PRF") == 0) || (scheme.ToUpper().CompareTo("CPR") == 0) || (scheme.ToUpper().CompareTo("PREFERRED") == 0))
            {
                mnem = "A:" + ticker.Substring(0, 4);
            }
            else
            {
                if (ticker.Length > 3)
                {
                    mnem = "A" + ticker.Substring(0, 3) + "X";
                }
                else
                {
                    mnem = "A:" + ticker.PadRight(3,' ') + "X";
                }
        ...
        {
            string[] endings = new string[] { "JSC", "JOINT STOCK COMPANY", "JOINT STOCK CO", "JOINT STOCK CORPORATION", "JOINT-STOCK CORPORATION", "PUBLIC LIMITED COMPANY", "INCORPORATED", "COMPANY", "LIMITED", "LTD", "CORPORATION", "CO.", "CORP", "INC", "LLC", "PLC.", "PLC", "Public Limited Company", "Public Limited Co", "Public Limited Co.", "CORPORATION LIMITED", "CORP. LTD.", "CORP LTD" };
            foreach (string ending in endings)
            {
                if ((nameLeft.Length > ending.Length) && ((nameLeft.Substring(nameLeft.Length - ending.Length - 1).Equals(" " + ending)) || (nameLeft.Substring(nameLeft.Length - ending.Length - 1).Equals("." + ending))))
                {
                    ...
                    else if (ending.Equals("CORPORATION LIMITED") || ending.Equals("CORP. LTD.") || ending.Equals("CORP LTD"))
                    {
                        nameSuffix = "CRL";
                    }
        ...
        private void GenerateFile(string lineToFile)
        {
            try
            {
                ...
                string fileName = string.Format("Australia_{0}.MAC", ConfigObj.Date);
