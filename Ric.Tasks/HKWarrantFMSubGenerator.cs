...
namespace Ric.Tasks
{
    public class HKWarrantFMSubGenerator
    {
        private StreamWriter swWarrantLog = null;
        ...
        private readonly string warrantMainPageUri = "http://www.hkex.com.hk/eng/dwrc/newissue/newlaunch.htm";
        ...
        public HKWarrantFMSubGenerator(HKFMAndBulkFileGenerator parent)
        {
            this.parent = parent;
        }
        ...
        {
            this.logger = logger;
            this.RicList = parent.RicListWarrant;
            this.RicChineseList = parent.ChineseListWarrant;
            this.taskResultList = taskResultList;
            this.outputPath = outputPath;

            if (!Directory.Exists(outputPath + "\\" + parent.RicTemplatePath + "\\" + parent.WarrantSubDir))
            {
                Directory.CreateDirectory(outputPath + "\\" + parent.RicTemplatePath + "\\" + parent.WarrantSubDir);
            }

            string swWarrantLogPath = outputPath + "\\" + parent.RicTemplatePath + "\\" + parent.WarrantSubDir +
                "\\" + parent.WarrantLogName ;
            swWarrantLog = new StreamWriter(swWarrantLogPath, false);
            swWarrantLog.AutoFlush = true;
            taskResultList.Add(new TaskResultEntry("WarrantLog", "Log File for Warrant", swWarrantLogPath));

            string swChineseNameLogPath = outputPath + "\\" + parent.RicTemplatePath + "\\" + parent.WarrantSubDir +
"\\" + "WarrantChineseNameLog_" + DateTime.Now.ToString("dd_MMM_yyyy", null) + ".txt";
            swChineseNameLog = new StreamWriter(swChineseNameLogPath, false);
            swChineseNameLog.AutoFlush = true;
            taskResultList.Add(new TaskResultEntry("WarrantChineseNameLog", "Chinese Name Log File for Warrant", swChineseNameLogPath));

            fmSerialNumberWarrant = parent.WarrantFmSn;
        }

        public void WarrantGenerate()
        {
            ...
            {
                GenerateWarrantTemplate(i * 20, RicList);
                fmSerialNumberWarrant = parent.UpdateFMSerialNumber(fmSerialNumberWarrant);
            }
            ...
            swWarrantLog.WriteLine("---------------------------------------------------------------------");
            swWarrantLog.WriteLine("Generated Ric#:" + RicList.Count);
            swWarrantLog.WriteLine("Execution Time:" + runningTime.ToString());
            swWarrantLog.Close();
        ...
        private void GenerateWarrantTemplate(int start, List<RicInfo> ricInfoList)
        {
            ...
            {
                logger.LogErrorAndRaiseException("Generate Warrant FM Files Error: " + ex.Message);
            }
        ...
        private HtmlAgilityPack.HtmlNodeCollection GetNewlyWarrantRicNode(string uri)
        {
            ...
            {
                logger.LogErrorAndRaiseException("There's no newly launched Warrant. Please go to " + uri + "to have a check.");
            }
            return ricNodeList;
        }
        ...
        {
            List<RicInfo> WarrantRicChineseList = new List<RicInfo>();
            HtmlAgilityPack.HtmlNodeCollection ricNodeList = GetNewlyWarrantRicNode("http://www.hkex.com.hk/chi/dwrc/newissue/newlaunch_c.htm");
            ...
            for (int i = parent.WarrantStartPos - 1; i < endPos; i++)
            {
                ...
                WarrantRicChineseList.Add(ricInfo);
            }
            return WarrantRicChineseList;
        }
