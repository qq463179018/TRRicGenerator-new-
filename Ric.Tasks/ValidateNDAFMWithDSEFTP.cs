...
namespace Ric.Tasks
{
    ...
    {
        private string txtFileFromFtp;
        private string fMFilePath;
        List<KoreaFMLine> listKFM = new List<KoreaFMLine>();
        private ValidateNDAFMWithDSEFTPConfig configObj;
        DateTime dateValue;
            ...
            try
            {
                ...
                while ((tmp = sr.ReadLine()) != null)
                {
                    if (tmp.Length>805)
                    {
                        ...
                        {
                            RIC = tmp.Substring(2, 20),
                            ISIN = tmp.Substring(83, 12),
                            Ticker = tmp.Substring(411, 25),
                            QACommonName = tmp.Substring(22, 36),
                            MatDate = tmp.Substring(781, 8),
                            StrikePrice = tmp.Substring(790, 14)
                        };
                        listKFM.Add(kFML);
                    }
