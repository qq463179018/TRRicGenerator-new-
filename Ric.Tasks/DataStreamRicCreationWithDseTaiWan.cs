...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDseTaiWan : GeneratorBase
    {
        #region Fields
        ...
        private void DownloadFtpFiles()
        {
            ...
            //特定的文件头
            string[] fileStartArr = new string[] { "0645","0126","1392","1127", "EM01", "EM11" };
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                if (item.fileType.StartsWith("EM"))
                {
                    ...
                    if (exchangeCode.CompareTo("TWO") != 0)
                    {
                        continue;
                    }
                }
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                bulkTemp.Mnem = "TW:" + GetMnem(item.CompanyName);
                ...
                if (item.marketSegmentName.CompareTo("Taiwan SE") == 0)
                {
                    bulkTemp.DefMki = "TA";
                    bulkTemp.Remk = "TASE";
                    bulkTemp.Mic = "TAI";
                ...
                else if (item.marketSegmentName.CompareTo("Gretai Securities") == 0)
                {
                    bulkTemp.DefMki = "TO";
                    bulkTemp.Remk = "TAOC";
                    bulkTemp.Mic = "XTO";
                }
                else if (item.marketSegmentName.CompareTo("GTSM Emerging Market") == 0)
                {
                    bulkTemp.DefMki = "TO";
                    bulkTemp.Remk = "TAOE";
                    bulkTemp.Mic = "XTO";
        ...
        private void GenerateFile(string lineToFile)
        {
            try
            {
                ...
                string fileName = string.Format("TaiWan_{0}.MAC", ConfigObj.Date);
