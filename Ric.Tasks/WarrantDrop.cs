...
namespace Ric.Tasks
{
    public class WarrantDrop : GeneratorBase
    {
        ...
        private KoreaCompanyWarrantDropGeneratorConfig configObj;
            ...
            try
            {
                ...
                if (nodeCollections.Count > 0)
                {
                    for (int i = nodeCollections.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (node != null)
                        {
                            titleNode = node.InnerText;
                        }
                        titleNode = titleNode.Contains("[") ? titleNode.Replace("[", "").Replace("]", "").Trim() : titleNode;
                        ...
                        if (titleNode.Contains("신주인수권증권 상장폐지"))
                        {
                            ...
                            string title = titleNode.Substring(0, "신주인수권증권 상장폐지".Length).Trim();
                            if (title.Equals("신주인수권증권 상장폐지"))
                            {
                                ...
                                {
                                    attribute = attribute.Split('(')[1].Split(')')[0].Trim(new Char[] { ' ', '\'', ';' });
                                }
        ...
        private void GenerateGEDAFile()
        {
            ...
            foreach (CompanyWarrantDropTemplate item in waDrop)
            {
                try
                {
                    string fileName = "KR_DROP_" + DateTime.Parse(item.EffectiveDate).ToString("yyyyMMdd", new CultureInfo("en-US")) + ".txt";
