...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDseSingapore : GeneratorBase
    {
        #region Fields
        ...
        private List<string> abnormalRecordFile = new List<string>();
        private List<string> exceptionRecord = new List<string>();
        ...
        {
            List<DseRecord> records;
            if (!ConfigObj.isTreatAbnormalRecord)
            {
                DownloadFtpFiles();
                records = GetRecords();
            }
        ...
        private void DownloadFtpFiles()
        {
            ...
            //特定的文件头
            string[] fileStartArr = new string[] { "0027", "1873","EM01","EM11" };
        ...
        private List<DseRecord> GetAbnormalRecords()
        {
            List<DseRecord> xeRecord = new List<DseRecord>();
            string Folder = Path.Combine(ConfigObj.OutputPath, DateTime.Today.ToString("yyyy-MM-dd"));
            Folder += @"\ExceptionRecord\";
            string fileName = string.Format(@"ExceptionRecord_{0}.M", ConfigObj.Date);
            ...
            using(StreamReader sr = new StreamReader(filePath))
            {
                ...
                while ((tmp = sr.ReadLine()) != null)
                {
                    ...
                    xeRecord.Add(new DseRecord(tmp,"abnormal"));
                    //xeRecord.Add(tmp);

                    string msg = string.Format("Get 1 abnormal record from file: {0}.", fileName);
                    Logger.Log(msg);
        ...
        private Dictionary<string, DseFieldPosition> InitializeDseFields()
        {
            ...
            dseFields.Add("Round Lot Size",new DseFieldPosition(786,805));
        ...
        private bool RicIsLegal(string ric)
        {
            if (ric.Length > 7)
            {
                int i = ric.LastIndexOf('.');
                if (i >= 2)
                {
                    if (ric.Substring(i - 2, 2).CompareTo("bi") == 0)
                    {
                        return false;
                    }
                }
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                item.roundLotSize = FormatField(record, "Round Lot Size", dseFields);
                item.marketSegmentName = FormatField(record, "Market Segment Name", dseFields);
                item.originalRecord = record.DseContent;
                item.fileType = record.DseFileType;

                if (item.fileType.StartsWith("EM"))
                {
                    string exchangeCode = item.ExchangeCode;
                    if (exchangeCode.CompareTo("SES") != 0)
                    {
                        continue;
                    }
                }

                if (item.ThomsonReutersClassificationScheme.CompareTo("RTS") != 0 && RicIsLegal(item.RIC))
                {
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                {
                    formatNameResult = FormatBulkNames(item.SecurityLongDescription, 24, 24, nameInputType.Name, ref name1, ref name2, ref suffix);
                    if (formatNameResult == -1)
                    {
                        exceptionRecord.Add(item.originalRecord);
                        continue;
                    }
                    ...
                    formatNameResult = FormatBulkNames(item.SecurityLongDescription, 19, 24, nameInputType.Fname, ref name1, ref name2, ref suffix);
                    if (formatNameResult == -1)
                    {
                        exceptionRecord.Add(item.originalRecord);
                        continue;
                ...
                {
                    formatNameResult = FormatBulkNames(item.CompanyName, 24, 24, nameInputType.Name, ref name1, ref name2, ref suffix);
                    if (formatNameResult == -1)
                    {
                        exceptionRecord.Add(item.originalRecord);
                        continue;
                    ...
                    formatNameResult = FormatBulkNames(item.CompanyName, 19, 24, nameInputType.Fname, ref name1, ref name2, ref suffix);
                    if (formatNameResult == -1)
                    {
                        exceptionRecord.Add(item.originalRecord);
                        continue;
                ...
                bulkTemp.Mnem2 = "T:" + item.Ticker.PadRight(10, ' ');
                ...
                if (item.marketSegmentName.CompareTo("Main Board") == 0)
                {
                    bulkTemp.Remk = "SMB ";
                }
                else if (item.marketSegmentName.CompareTo("Catalist") == 0)
                {
                    bulkTemp.Remk = "SS  ";
                }
                else
                ...
                if (item.RIC.Length > 7)
                {
                    int i = item.RIC.LastIndexOf('.');
                    if (i >= 2)
                    {
                        if (item.RIC.Substring(i - 2, 2).CompareTo("ol") == 0)
                        {
        ...
        private string GetMnem(string ric)
        {
            if (ric.Length > 7)
            {
                int i = ric.LastIndexOf('.');
                if (i >= 2)
                {
                    if (ric.Substring(i - 2, 2).CompareTo("ol") == 0)
                    {
                        return "T:" + ric.Substring(0, 3) + "O";
                    }
                    else
                    {
                        return "T:" + ric.Substring(0, 4);
                    }
                }
        ...
        private int FormatBulkNames(string companyName, int nameLength1, int nameLength2, nameInputType type, ref string name1, ref string name2, ref string suffix)
        {
        ...
        private void GenerateFile(string lineToFile)
        {
            try
            {
                ...
                string fileName = string.Format("Singapore_{0}.MAC", ConfigObj.Date);
                ...
                if (exceptionRecord.Count != 0)
                {
                    outputFolder += @"\ExceptionRecord\";
                    if (!Directory.Exists(outputFolder))
                    {
                        Directory.CreateDirectory(outputFolder);
                    }
                    fileName = string.Format(@"ExceptionRecord_{0}.M",ConfigObj.Date);
                    filePath = Path.Combine(outputFolder, fileName);
                    string comtent = string.Join("\r\n", exceptionRecord.ToArray());
                    File.WriteAllText(filePath, comtent, Encoding.ASCII);
                    abnormalRecordFile.Add(filePath);
                }
        ...
        private void SendMail()
        {
            try
            {
                ...
                if (exceptionRecord.Count == 0)
                {
                    subject = "From Task RIC Creation Using DSE - Singapore " + DateTime.Now.ToString("dd-MMM-yyyy") + "[task complete]";
                    content = "<center>*******Task RIC Creation Using DSE - Singapore complete .*********</center><br />";
                    SendMail(service, subject, content, new List<string>());//ok
                    return;
                }
                else
                {
                    subject = "From Task Task RIC Creation Using DSE - Singapore " + DateTime.Now.ToString("dd-MMM-yyyy") + "[exist abnormal records]";
                    content = "<center>*******Task RIC Creation Using DSE - Singapore exist abnormal records, please modify those records. *********</center><br />";

                    SendMail(service, subject, content, abnormalRecordFile);//ok
                    return;
                }
