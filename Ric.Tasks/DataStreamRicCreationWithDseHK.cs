...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDseHK : GeneratorBase
    {
        #region Fields
        ...
        private void DownloadFtpFiles()
        {
            ...
            //特定的文件头
            string[] fileStartArr = new string[] { "0001","EM01","EM11" };
            //string fileStart = "0001";
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                if (item.fileType.StartsWith("EM"))
                {
                    ...
                    if (exchangeCode.CompareTo("HKG") != 0)
                    {
                        continue;
                    }
                }
                
                if (!item.SecurityDescription.Contains("(TEMP)")&&(!string.IsNullOrEmpty(item.CompanyName)))
                {
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                bulkTemp.Mnem = "K:" + GetMnem(item.CompanyName);
                ...
                if (item.SecurityDescription.EndsWith("H"))
                {
                    if (bulkTemp.Name2.CompareTo(string.Empty.PadRight(24,' '))==0)
                    {
                        bulkTemp.Name1 = bulkTemp.Name1.Substring(0, 23) + "H";
                    }
                    else
                    {
                        bulkTemp.Name2 = bulkTemp.Name2.Substring(0, 23) + "H";
                    }

                    bulkTemp.CName = bulkTemp.CName.Substring(0, 23) + "H";
                }
                
                if (item.CurrencyCode.CompareTo("CNY") == 0)
                {
                    if (bulkTemp.Name2.CompareTo(string.Empty.PadRight(24, ' ')) == 0)
                    {
                        bulkTemp.Name1 = bulkTemp.Name1.Substring(0, 21) + "RMB";
                    }
                    else
                    {
                        bulkTemp.Name2 = bulkTemp.Name2.Substring(0, 21) + "RMB";
                    }

                    bulkTemp.CName = bulkTemp.CName.Substring(0, 23) + "RMB";
                }
                ...
                bulkTemp.DefMki = "HK";
                bulkTemp.Nosh = "1";
                if (string.IsNullOrEmpty(item.Ticker.Trim()))
                {
                    bulkTemp.Mnem2 = "HK" + string.Empty.PadRight(10, ' ');
                }
                else
                {
                    bulkTemp.Mnem2 = "HK" + item.Ticker.TrimStart(new char[] { '0' }).PadRight(10, ' ');
                }
                ...
                if (item.CurrencyCode.CompareTo("HKD") == 0 && item.SecurityDescription.EndsWith("H"))
                {
                    bulkTemp.Geog = "051";
                    bulkTemp.Dcur = "079";
                }
        ...
        private string GetMnem(string companyName)
        {
            ...
            else if (tmp.Length == 3)
            {
                ...
                mnem += tmp[2].Substring(tmp[2].Length-1,1);
            }
            else if (tmp.Length == 2)
            {
                ...
                {
                    mnem += tmp[i].Substring(0, 1);
                    mnem += tmp[i].Substring(tmp[i].Length-1,1);
                }
            }
            else
            {
        ...
        //{
        //    string[] endings = new string[] { "JSC", "JOINT STOCK COMPANY", "JOINT STOCK CO", "JOINT STOCK CORPORATION", "JOINT-STOCK CORPORATION", "PUBLIC LIMITED COMPANY", "INCORPORATED", "COMPANY", "LIMITED", "LTD", "CORPORATION", "CO.", "CORP", "INC" };
        ...
        private string TrimCompanyEndings(string nameLeft, ref string nameSuffix)
        {
            string[] endings = new string[] { "JSC", "JOINT STOCK COMPANY", "JOINT STOCK CO", "JOINT STOCK CORPORATION", "JOINT-STOCK CORPORATION", "PUBLIC LIMITED COMPANY", "INCORPORATED", "COMPANY", "LIMITED", "LTD","LTD.", "CORPORATION", "CO.", "CORP", "INC", "LLC", "PLC.", "PLC", "Public Limited Company", "Public Limited Co", "Public Limited Co." };
            foreach (string ending in endings)
            {
                if ((nameLeft.Length > ending.Length) && ((nameLeft.Substring(nameLeft.Length - ending.Length - 1).Equals(" " + ending)) || (nameLeft.Substring(nameLeft.Length - ending.Length - 1).Equals("." + ending))))
                {
                    ...
                    else if (ending.Equals("LIMITED") || ending.Equals("LTD") || ending.Equals("LTD."))
                    {
                        nameSuffix = "LTD.";
                    }
        ...
        private void GenerateFile(string lineToFile)
        {
            try
            {
                ...
                string fileName = string.Format("HK_{0}.MAC", ConfigObj.Date);
