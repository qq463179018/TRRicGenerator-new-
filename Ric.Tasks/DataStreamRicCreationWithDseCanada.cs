...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDseCanada : GeneratorBase
    {
        #region Fields
        ...
        private void DownloadFtpFiles()
        {
            ...
            string[] fileStartArr = new string[] { "0082", "0117", "1273", "2032", "2029", "EM03", "EM13" };
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                if (item.SecurityLongDescription.Contains("Debentures") || item.SecurityLongDescription.Contains("Cumulative * Pref Shs"))
                {
                    continue;
                }
                if (item.fileType.StartsWith("EM"))
                {
                    ...
                    if (exchangeCode.CompareTo("TOR") != 0 && exchangeCode.CompareTo("CVE") != 0 && exchangeCode.CompareTo("NEX") != 0 && exchangeCode.CompareTo("CNX") != 0)
                    {
                        continue;
                    }
                }
        ...
        private string FormatField(DseRecord record, string field, Dictionary<string, DseFieldPosition> dseFields)
        {
            ...
            string result = record.DseContent.Substring(dseField.StartPosition - 1, dseField.EndPosition - dseField.StartPosition + 1).Trim().ToUpper().ToUpper();
            return result;
        }
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                {
                    string tick = item.Ticker.Replace(".","");
                    tick = tick.Replace(" ","");
                    bulkTemp.Bdate = "C:" + tick.PadRight(10, ' ');
                ...
                if (item.ExchangeCode.CompareTo("TOR") == 0)
                {
                    bulkTemp.DefMki = "TR";
                }
                else if(item.ExchangeCode.CompareTo("CNX") == 0)
                {
                    bulkTemp.DefMki = "CN";
                }
                else
                ...
                else if (item.ExchangeCode.CompareTo("CVE") == 0)
                {
                    bulkTemp.Remk = "CDNX";
                    bulkTemp.Mic = "TSX";
                }
                else if (item.ExchangeCode.CompareTo("NEX") == 0)
                {
                    bulkTemp.Remk = "NEX ";
                    bulkTemp.Mic = "TSX";
                }
                ...
                if (item.CUSIP.Length>11)
                {
                    bulkTemp.Cusip = "Q" + item.CUSIP.Substring(0, 11);
                }
                else
                {
                    bulkTemp.Cusip = "Q" + item.CUSIP.Substring(0,item.CUSIP.Length-1).PadRight(11, ' ');
                }
        ...
        private string GetMnem(string ticker)
        {
            ...
            if (ticker.Length > 4)
            {
                mnem = "C:" + ticker.Substring(0,3);
                mnem += ticker.Substring(ticker.Length-1,1);
            }
            else
            {
                mnem = "C:"+ticker.PadRight(4, ' ');
            }
        ...
        {
            string[] endings = new string[] { "JSC", "JOINT STOCK COMPANY", "JOINT STOCK CO", "JOINT STOCK CORPORATION", "JOINT-STOCK CORPORATION", "PUBLIC LIMITED COMPANY", "INCORPORATED", "COMPANY", "LIMITED", "LTD", "CORPORATION", "CO.", "CORP","CORP.", "INC", "LLC","PLC.", "PLC", "Public Limited Company", "Public Limited Co", "Public Limited Co." };
            foreach (string ending in endings)
            {
                if ((nameLeft.Length > ending.Length) && ((nameLeft.Substring(nameLeft.Length - ending.Length - 1).Equals(" " + ending)) || (nameLeft.Substring(nameLeft.Length - ending.Length - 1).Equals("." + ending))))
                {
                    ...
                    else if (ending.Equals("CORPORATION") || ending.Equals("CORP") || ending.Equals("CORP."))
                    {
                        nameSuffix = "CORP.";
                    }
        ...
        private void GenerateFile(string lineToFile)
        {
            try
            {
                ...
                string fileName = string.Format("Canada_{0}.MAC", ConfigObj.Date);
