...
namespace Ric.Tasks
{
    public class ELWFMSecondPart : GeneratorBase
    {
        ...
        protected override void Start()
        {
            ...
            //TaskResultList.Add(new TaskResultEntry("LOG FILE", "LOG", Logger.FilePath, FileProcessType.Log));
        }
        ...
        public void StartELWFMSecondPartJob()
        {
            ...
            if (kobaList.Count > 0)
            {
                TaskResultList.Add(new TaskResultEntry("KOBA FM Folder", "KOBA FM Folder", configObj.FM));
            ...
            {
                TaskResultList.Add(new TaskResultEntry("ELW FM2 Folder", "ELW FM2 Folder", configObj.FM));
                TaskResultList.Add(new TaskResultEntry("BulkFile Folder", "BulkFile Folder", configObj.BulkFile));
            ...
            try
            {
                ...
                foreach (var item in fmSecondList)
                {
                    ...
                    if (fmSecondHash.Contains(item.RIC))
                    {
                        ...
                        ((Range)wSheet.Cells[startLine, 10]).NumberFormat = "@";
                        wSheet.Cells[startLine, 10] = kw.MatDate;
                        ...
                        ((Range)wSheet.Cells[startLine, 14]).NumberFormat = "@";
                        wSheet.Cells[startLine, 14] = kw.IssueDate;
        ...
        private void GeneratePriceChangeCsv()
        {
            ...
            TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(path), "EMA File", path, FileProcessType.Other));
        ...
        private void GeneretaNoFm1EmaFile()
        {
            ...
            if (emaRes.Count > 1)
            {
                ...
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(filePath), "EMA File", filePath, FileProcessType.Other));
            }
        }
        //need to be confirm
        private void GenerateFM2BcuUpdate(string gedaDir)
        {
            ...
            if (res.Count > 1)
            {
                ...
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(filePath), "GEDA File", filePath, FileProcessType.GEDA_BULK_RIC_UPDATE));
            }
        ...
        private void GenerateISINChangeRicCreation(string gedaDir)
        {
            ...
            {
                OperateExcel.WriteToTXT(filePath, res);
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(filePath), "GEDA File(ADD)", filePath, FileProcessType.GEDA_BULK_RIC_CREATION));

            }
            if (dropRes.Count > 1)
            {
                OperateExcel.WriteToTXT(dropFilePath, dropRes);
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(dropFilePath), "GEDA File(DROP)", dropFilePath, FileProcessType.GEDA_BULK_RIC_DELETE));
            }
        }
        private void GenerateBcuRicRemove(string gedaDir)
        {
            ...
            {
                OperateExcel.WriteToTXT(filePath, res);
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(filePath), "GEDA File(BCU_RIC_REMOVE)", filePath, FileProcessType.GEDA_BULK_BCU_CON_DELETE));
            }
        }
        ...
        private void GenerateFM2MdnUpdate(string gedaDir)
        {
            ...
            TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(filePath), "GEDA File(MD_N_UPDATE)", filePath, FileProcessType.GEDA_BULK_RIC_CHANGE));
        ...
        private void GenerateQAChgFirstTradeDate(string NdaDir)
        {
            ...
            if (res.Count > 1)
            {
                ...
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(filePath), "NDA QA File", filePath, FileProcessType.NDA));
            }
        }
        ...
        private void GenerateIAChgCommonNameCsv(string NdaDir)
        {
            ...
            {
                OperateExcel.WriteToCSV(filePath, res);
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(filePath), "NDA IA File", filePath, FileProcessType.NDA));
            }
        ...
        private void GenerateFmSecondPartEmail()
        {
            string filename = "Korea FM for " + DateTime.Today.ToString("dd-MMM-yy").Replace("-", " ") + " (Afternoon).xls";
            string ipath = Path.Combine(configObj.FM, filename);
            TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(ipath), "FM File", ipath, CreateMail()));
        }
        #endregion
        ...
        private void GeneretaKobaEmaFile()
        {
            ...
            {
                OperateExcel.WriteToCSV(filePath, emaRes);
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(filePath), "EMA File(KOBA)", filePath, FileProcessType.Other));
            }
        }
        ...
        private void GenerateKobaCreation(string gedaDir)
        {
            ...
            {
                OperateExcel.WriteToTXT(filePath, res);
                TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(filePath), "GEDA File(KOBA)", filePath, FileProcessType.GEDA_BULK_RIC_UPDATE));
            }
        }
        ...
        private void GenerateKobaIA(string ndaDir)
        {
            ...
            OperateExcel.WriteToCSV(filePath, res);
            TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(filePath), "NDA IA File(KOBA)", filePath, FileProcessType.NDA));
        }
        private void GenerateKobaQA(string ndaDir)
        {
            ...
            OperateExcel.WriteToCSV(filePath, res);
            TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(filePath), "NDA QA File(KOBA)", filePath, FileProcessType.NDA));
        }
        ...
        private void GenerateKobaEmail()
        {
            string filename = "Korea FM (KOBA Add)" + DateTime.Today.ToString("dd-MMM-yy").Replace("-", " ") + " (Afternoon).xls";
            string fpath = Path.Combine(configObj.FM, filename);
            TaskResultList.Add(new TaskResultEntry(Path.GetFileNameWithoutExtension(fpath), "FM File(KOBA)", fpath, CreatKobaMail()));
        }
        #endregion
        ...
        private void GenerateNewUnderlyingFiles()
        {
            ...
            foreach (KoreaUnderlyingInfo newItem in newUnderLying)
            {
                ...
                TaskResultList.Add(new TaskResultEntry(Path.GetFileName(fileName), Path.GetFileNameWithoutExtension(fileName), fileName, FileProcessType.Other));
            ...
            if (superKQ)
            {
                TaskResultList.Add(new TaskResultEntry(superKQFileName, superKQFileName, Path.Combine(filePath, superKQFileName), FileProcessType.Other));
            }
            if (superKS)
            {
                TaskResultList.Add(new TaskResultEntry(superKSFileName, superKSFileName, Path.Combine(filePath, superKSFileName), FileProcessType.Other));
            }
        }
