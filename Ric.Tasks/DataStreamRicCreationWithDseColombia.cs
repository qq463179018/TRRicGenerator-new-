...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDseColombia : GeneratorBase
    {
        #region Fields
        ...
        private void DownloadFtpFiles()
        {
            ...
            //特定的文件头
            string[] fileStartArr = new string[] { "0119", "EM03", "EM13" };
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                if (item.fileType.StartsWith("EM"))
                {
                    ...
                    if (exchangeCode.CompareTo("COL") != 0)
                    {
                        continue;
                    }
                }
        ...
        private string GetMnem(string ric)
        {
            ...
            if (ric.Length > 6)
            {
                mnem = ric.Substring(0, 2);
                mnem += ric.Substring(ric.Length - 1, 1);
            }
            else if (ric.Length >= 3 && ric.Length <= 6)
            {
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                bulkTemp.Mnem = "CB:" + GetMnem(item.RIC);
        ...
        private void GenerateFile(string lineToFile)
        {
            try
            {
                ...
                string fileName = string.Format("Colombia_{0}.MAC", ConfigObj.Date);
