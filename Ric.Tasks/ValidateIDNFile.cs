...
namespace Ric.Generator.Lib.Validation
{
    ...
    {
        private static ValidateIDNFileConfig configObj;
        List<string> listRic;
        ...
        private void GetDataFromGatsToListList(List<string> listRic, string strPatternGATS, List<List<string>> listListRic)
        {
            ...
            if (listRic.Count == 1 && string.IsNullOrEmpty(listRic[0].Trim()))
            {
                return;
            }
            ...
            if (qiuShang > 0)
            {
                for (int i = 0; i < qiuShang; i++)
                {
                    ...
                    {
                        string strTmp = listRic[i * fenMu + j].Trim();
        ...
        private void GetDataFromGATS(string strQuery, List<List<string>> listListRic, string strPatternGATS)
        {
            ...
            if (!string.IsNullOrEmpty(response))
            {
                ...
                foreach (Match match in matches)
                {
                    ...
                    List<string> li = new List<string> {ric, prov, off};
                    listListRic.Add(li);
                }
            }
            else
        ...
        private void GenerateFile(IEnumerable<List<string>> listList, string strSaveFilePath)
        {
            if (listList != null)
            {
