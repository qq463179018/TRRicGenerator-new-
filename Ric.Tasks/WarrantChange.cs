...
{    
    ...
    {       
        ...
        private KOREACompanyWarrantChangeGeneratorConfig configObj;
        ...
        protected override void Initialize()
        {
            ...
            holidayList = HolidayManager.SelectHoliday(MarketId) ?? new List<DateTime>();
        }       
        ...
        private Object[] ReturnTargetObjArray(HtmlNode titleNode, string type)
        {
            ...
            {
                string pageSource = null;
                string attribute = titleNode.Attributes["onclick"].Value.Trim();
                char[] trimChar = { ' ', '\'', ',', '(', ')' };
                if (!string.IsNullOrEmpty(attribute))
                {
                    attribute = attribute.Split('(')[1].Split(')')[0].Trim(trimChar);
                    string param1 = attribute.Split(',')[0].Trim(trimChar);
                    string param2 = attribute.Split(',')[1].Trim(trimChar);
                    ...
                    if (doc != null)
                    {
                        ticker = doc.DocumentNode.SelectSingleNode(".//header/h1").InnerText.Trim();
                    ...
                    if (!string.IsNullOrEmpty(strPre))
                    {
                        ...
                        if (preArr.Length > 1)
                        {
                            for (var i = 2; i < preArr.Length; i++)
                            {
                                ...
                                string koreaName = strPre.Substring(strOnePos, (strTwoPos - strOnePos)).Trim();    //split by ':'[1]
                                koreaName = koreaName.Split(':')[1].Trim();
                                if (koreaName.Contains("(주)"))
                                    koreaName = koreaName.Replace("(주)", "");
                                string quantity = strPre.Substring(strTwoPos, (strThreePos - strTwoPos)).Trim();  //split by  '\n'[1]
                                quantity = quantity.Split('\n')[1].Trim();
                                string effectiveDate = strPre.Substring(strFourPos, (strFivePos - strFourPos)).Trim();     //split by ':'[1]
                                effectiveDate = effectiveDate.Split(':')[1].Trim();
                                string exercisePrice = strPre.Substring(strSixPos, (strSevenPos - strSixPos)).Trim().Replace("원", "").Replace(",", "");     //split by ':'[1]
                                exercisePrice = exercisePrice.Split(':')[1].Trim();
                                string expiryDate = strPre.Substring(strSevenPos, (strEightPos - strSevenPos)).Trim();             //split by ':'[1]
                                expiryDate = expiryDate.Split(':')[1].Trim();
                                string isinKoreaCode = strPre.Substring((strEightPos + 8)).Trim();
                                ...
                                string oldQuantity = tempQuantity.Split('→')[0].Trim(trimChar);
                                ...
                                if (isinKoreaCode.Contains("표준코드") || isinKoreaCode.Contains("단축코드"))
                                {
                                    ...
                                    isin = isinKoreaCode.Substring((isinPos), (koreacodePos - isinPos)).Trim(new[] { '(', ':', ' ', ',' });
                                    koreaCode = isinKoreaCode.Substring((koreacodePos + "단축코드".Length)).Trim(new[] { ')', ':', ' ', ',' });
                                }
                                ...
                                {
                                    fromDate = expiryDate.Split('~')[0].Trim();
                                    endDate = expiryDate.Split('~')[1].Trim();
            ...
            try
            {
                ...
                if (doc != null)
                {
                    ...
                    if (tableNodes.Count > 0)
                    {
                        ...
                        {
                            string strIsin = item.SelectSingleNode(".//tr[2]/td[2]").InnerText.Trim();
                            string strCode = item.SelectSingleNode(".//tr[2]/td[3]").InnerText.Trim();

                            if (strIsin.Equals("표준코드") && strCode.Equals("단축코드"))
                            {
                                string koreanName = item.SelectSingleNode(".//tr[1]/td[2]").InnerText.Trim();
                                string isin = item.SelectSingleNode(".//tr[3]/td[1]").InnerText.Trim();
                                string koreaCode = item.SelectSingleNode(".//tr[3]/td[2]").InnerText.Trim();
                                string quantityOfWarrant = item.SelectSingleNode(".//tr[5]/td[2]").InnerText.Trim();
                                string effectiveDate = item.SelectSingleNode(".//tr[8]/td[2]").InnerText.Trim();
                                string kqPre = item.SelectSingleNode(".//tr[10]/td[2]").InnerText.Trim();
                                ...
                                if (kqPre.Contains("행사가"))
                                {
                                    ...
                                    string strExercise = kqPre.Substring(strPos).Trim(new[] { ')', ' ', ':' });
                                    previousExercisePrice = Regex.Split(strExercise, "->", RegexOptions.IgnoreCase)[0].Trim().Replace("원", "");
                                    exercisePrice = Regex.Split(strExercise, "->", RegexOptions.IgnoreCase)[1].Trim().Replace("원", "");
                                }
        ...
        private void GetPriceList()
        {
            ...
            {
                Logger.Log("Error found in Getting price list from KRX website." + ex, Logger.LogType.Error);
            }
        }
        ...
        private void FormatData()
        {
            foreach (CompanyWarrantTemplate item in wcList)
            {
                try
                {
                    ...
                    System.Data.DataTable dt = ManagerBase.Select(ETI_KOREA_COMPANYWARRANT_TABLE_NAME, new[] { "*" }, condition);
        ...
        private void GenerateNewConvertionRatio(CompanyWarrantTemplate item)
        {
            try
            {
                ...
                if (oldRatio.Contains("%"))
                {
                    ...
                    string result = ((oldPrice * tempRatio) / newPrice).ToString("##.#####%").TrimEnd(new[] { '0', ' ' });
                    item.ConversionRatio = result;
                }
                else
                {
                    double ratio = Convert.ToDouble(oldRatio) * oldPrice / newPrice;
                    string result = ratio.ToString("##.#####%").TrimEnd(new[] { '0', ' ' });
                    item.ConversionRatio = result;
                }
            }
            catch (Exception ex)
        ...
        private void GenerateFMFiles()
        {
            ...
            foreach (CompanyWarrantTemplate item in wcList)
            {
                ...
                {                  
                    ...
                    ((Range)wSheet.Columns["A", Type.Missing]).ColumnWidth = 28;
                    ((Range)wSheet.Columns["B", Type.Missing]).ColumnWidth = 3;
                    ((Range)wSheet.Columns["C", Type.Missing]).ColumnWidth = 45;
                    ...
                    CompanyWarrantTemplate cwTemp = item;
                    ...
                    wSheet.Cells[7, 2] = ":";
                    ((Range)wSheet.Cells[7, 3]).Font.Color = ColorTranslator.ToOle(Color.Blue);
                    ...
                    wSheet.Cells[9, 3] = cwTemp.KoreaCode.Trim();
                    wSheet.Cells[10, 1] = "ISIN";
                    wSheet.Cells[10, 2] = ":";
                    ((Range)wSheet.Cells[10, 3]).Font.Color = ColorTranslator.ToOle(Color.Red);
                    ...
                    if (item.ChangeItems.Count != 0)
                    {
                        ...
                        {
                            ExcelUtil.GetRange("A" + index + ":C" + index, wSheet).Interior.Color = ColorTranslator.ToOle(Color.Yellow);
                        }
                    }
        ...
        private void CreateNDAQAFile(CompanyWarrantTemplate item, string dir)
        {
            ...
            try
            {
                ...
                strData.Append(item.RIC + "," + item.QACommonName + "," + item.ExercisePrice + "\r\n");
                strData.Append(ricF + "," + item.QACommonName + "," + item.ExercisePrice + "\r\n");
                File.WriteAllText(path, strData.ToString(), Encoding.UTF8);
                if (!filesResultList.Contains(fileName))
                {
        ...
        private void SetTaskResultList()
        {
            ...
            foreach (string keyRusult in keysArr)
            {
                ...
                {
                    TaskResultList.Add(new TaskResultEntry(keyRusult, resultType, filePath, FileProcessType.GEDA_BULK_RIC_CHANGE));
                }
                else if (resultType == "NDA IA FILE" || resultType == "NDA QA FILE")
                {
                    TaskResultList.Add(new TaskResultEntry(keyRusult, resultType, filePath, FileProcessType.NDA));
                }
                else
                {
                    TaskResultList.Add(new TaskResultEntry(keyRusult, resultType, filePath));
                }
            }                   
        }
