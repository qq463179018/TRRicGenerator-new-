...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDseKorea : GeneratorBase
    {
        #region Fields
        ...
        private void DownloadFtpFiles()
        {
            ...
            //特定的文件头
            string[] fileStartArr = new string[] { "0184", "1397", "0673", "1398", "3073", "EM01", "EM11" };
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                if (item.fileType.StartsWith("EM"))
                {
                    ...
                    if (exchangeCode.CompareTo("KSE") != 0 && exchangeCode.CompareTo("KOE") !=0)
                    {
                        continue;
                    }
                }
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                bulkTemp.Mnem = "KO:" + GetMnem(item.CompanyName);
                ...
                if (item.ExchangeCode.CompareTo("KSC") == 0)
                {
                    bulkTemp.DefMki = "SE";
                }
                else if (item.ExchangeCode.CompareTo("KOE") == 0)
                {
                    bulkTemp.DefMki = "SL";
                }
                else
        ...
        private void GenerateFile(string lineToFile)
        {
            try
            {
                ...
                string fileName = string.Format("Korea_{0}.MAC", ConfigObj.Date);
