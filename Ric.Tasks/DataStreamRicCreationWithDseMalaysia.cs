...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDseMalaysia : GeneratorBase
    {
        #region Fields
        ...
        private void DownloadFtpFiles()
        {
            ...
            //特定的文件头
            string[] fileStartArr = new string[] { "0049","EM01", "EM11" };
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                if (item.fileType.StartsWith("EM"))
                {
                    ...
                    if (exchangeCode.CompareTo("KLS") != 0)
                    {
                        continue;
                    }
                }
        ...
        private string GetMnem(string ric)
        {
            ...
            if (ric.Length > 7)
            {
                mnem = ric.Substring(0, 3);
                int i = ric.LastIndexOf('.');
                if (i > 0)
                {
                    mnem += ric.Substring(i - 1, 1);
                }
                else
                {
                    mnem += ' ';
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                bulkTemp.Mnem = "L:" + GetMnem(item.RIC);
                ...
                {
                    DateTime dt = DateTime.Parse(item.FirstTradingDate);
                    dt = dt.AddDays(-1);
                    string bdate = dt.ToString("yyyyMMdd");
                    bdate = bdate.Substring(6, 2) + "/" + bdate.Substring(4, 2) + "/" + bdate.Substring(2, 2);                   
                ...
                bulkTemp.Nosh = "1";
                bulkTemp.Mnem2 = "L" + item.malaysiaCode.PadRight(11, ' ');
        ...
        private void GenerateFile(string lineToFile)
        {
            ...
            try
            {
                ...
                string fileName = string.Format("Malaysia_{0}.MAC", ConfigObj.Date);
