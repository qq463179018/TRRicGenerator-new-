...
namespace Ric.Tasks
{
    class DataStreamRicCreationWithDseArgentina : GeneratorBase
    {
        #region Fields
        ...
        private void DownloadFtpFiles()
        {
            ...
            //特定的文件头
            string[] fileStartArr = new string[] { "0269", "EM03", "EM13" };
        ...
        private List<DataStreamRicCreationInfo> FormatRecords(List<DseRecord> records)
        {
            ...
            foreach (DseRecord record in records)
            {
                ...
                if (item.fileType.StartsWith("EM"))
                {
                    ...
                    if (exchangeCode.CompareTo("BUE") != 0)
                    {
                        continue;
                    }
                }
                string scheme = item.ThomsonReutersClassificationScheme;
                if (scheme.CompareTo("RTS") != 0 && scheme.CompareTo("CEDEAR") != 0 && scheme.CompareTo("ETF") != 0 && scheme.CompareTo("CEF") != 0)
                {
        ...
        private string GetMnem(string ticker,string ric)
        {
            ...
            if (string.IsNullOrEmpty(ticker))
            {
                ...
                if (i >= 3)
                {
                    mnem = ric.Substring(i - 3, 3);
                }
                else
                {
                    mnem = string.Empty.PadRight(3, ' ');
                }
                return mnem;
            }
            ...
            if (ticker.Length >= 3)
            {
                mnem = ticker.Substring(0, 2);
                mnem += ticker.Substring(ticker.Length - 1, 1);
            }
            else
            {
                mnem = ticker.PadRight(3, ' ');
            }
            return mnem;
        }
        ...
        {
            string[] endings = new string[] { "JSC", "JOINT STOCK COMPANY", "JOINT STOCK CO", "JOINT STOCK CORPORATION", "JOINT-STOCK CORPORATION", "PUBLIC LIMITED COMPANY", "INCORPORATED", "COMPANY", "LIMITED", "LTD", "CORPORATION", "CO.", "CORP", "INC", "LLC", "TBK", "TERBUKA", "PLC.", "PLC", "Public Limited Company", "Public Limited Co", "Public Limited Co.","SA","S.A." };
            foreach (string ending in endings)
            {
                if ((nameLeft.Length > ending.Length) && ((nameLeft.Substring(nameLeft.Length - ending.Length - 1).Equals(" " + ending)) || (nameLeft.Substring(nameLeft.Length - ending.Length - 1).Equals("." + ending))))
                {
                    ...
                    else if (ending.Equals("SA"))
                    {
                        nameSuffix = "SA";
                    }
                    else if (ending.Equals("S.A."))
                    {
                        nameSuffix = "S.A.";
                    }
                }
        ...
        private string FormatOutputLine(List<DataStreamRicCreationInfo> ricCreations)
        {
            ...
            foreach (DataStreamRicCreationInfo item in ricCreations)
            {
                ...
                bulkTemp.Mnem = "AG:" + GetMnem(item.Ticker,item.RIC);
                ...
                bulkTemp.Nosh = "1";
                bulkTemp.Mnem2 = "AR:" + item.Ticker.PadRight(9, ' ');
        ...
        private void GenerateFile(string lineToFile)
        {
            try
            {
                ...
                string fileName = string.Format("Argentina_{0}.MAC", ConfigObj.Date);
